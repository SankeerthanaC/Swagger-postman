...
public class MainController {
	...
	public String get(@PathVariable Integer id,Model model,RedirectAttributes ra)
	{
		...
		if(prod.isPresent())
		{
			...
			model.addAttribute("pageTitle", "Edit Medicine (ID: " +id+ ")");
			return "admin/prod_form";
		}
		throw new ProductNotFoundException("Could Not Found any Medicine with ID "+id);
	...
	public String deletecust(@PathVariable Integer id,RedirectAttributes ra)
	{
		...
		{
			throw new ProductNotFoundException("Could not find Medicine with ID "+id);
		}
		ProdRepo.deleteById(id);
		ra.addFlashAttribute("message","The Medicine with ID "+id+" has been deleted");
		}
		catch(ProductNotFoundException e)
		{
			ra.addFlashAttribute("message",e.getMessage());
